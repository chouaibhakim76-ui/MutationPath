    ...
    <section className="relative py-32 px-6">
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        {impacts.map((item, i) => (
          <motion.div
            key={item.headline}
            className="impact-card overflow-hidden"
            initial={{ opacity: 0, y: 40 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: false, margin: '-50px' }}
            transition={{ duration: 0.7, delay: i * 0.15 }}
            style={{
              background: `linear-gradient(135deg, ${item.color}0c 0%, rgba(6,6,18,0.9) 100%)`,
              border: `1px solid ${item.color}22`,
              boxShadow: `0 4px 32px rgba(0,0,0,0.4), inset 0 1px 0 ${item.color}18`,
            }}
            whileHover={{ boxShadow: `0 16px 60px rgba(0,0,0,0.5), 0 0 40px ${item.color}18, inset 0 1px 0 ${item.color}28` }}
          >
            {/* Top shimmer line */}
            <div
              className="h-[2px]"
            ...
            <div className="p-8 md:p-10">
              <div className="grid md:grid-cols-[auto_1fr] gap-8 items-center">
                ...
                <div className="flex md:flex-col items-center md:items-start gap-6 md:gap-3 md:min-w-[180px]">
                  ...
                  {/* Big animated number */}
                  <div>
                    <div style={{ color: item.color, textShadow: `0 0 30px ${item.color}60` }}>
